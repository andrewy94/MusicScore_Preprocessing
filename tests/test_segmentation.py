...
from pipeline.segmentation import find_segments, full_segmentation
...
# simple image
def test_full_segmentation_low_depth():
    # setup
    ...
    act_bboxes = full_segmentation(dummy_img)

    assert act_bboxes == exp_bboxes, f"Expected bboxes {exp_bboxes}, but got {act_bboxes}"

# complex image   
def test_full_segmentation_moderate_depth():
    # setup
    dummy_img = np.zeros((1000, 1000), dtype = np.uint8)
    dots = [
        (500, 500),
